...
describe('Prueba de integracion REAL API EstudianteCurso', () => {
  let profesorToken;
  let estudianteToken;
  let profesorUser;
  let estudianteUser;
  ...
  describe('POST /api/estudiante-curso/asignar', () => {
    ...
    it('Si todo el flujo es correcto debe retornar 201 y asignar el estudiante', async () => {
      const response = await request(app)
        .post('/api/estudiante-curso/asignar')
        .set('Authorization', `Bearer ${profesorToken}`)
        .send({ refCurso: curso.id, refEstudiante: estudianteUser.id });

      expect(response.status).toBe(201);
      expect(response.body.mensaje).toBe(
        'Estudiante asignado al curso exitosamente'
      );
      expect(response.body.asignacion.refCurso).toBe(curso.id);
  ...
  describe('GET /api/estudiante-curso/curso/:refCurso', () => {
    it('Si el usuario tiene acceso debe retornar 200 y la lista de estudiantes', async () => {
      await prisma.estudianteCurso.create({
        data: { refCurso: curso.id, refEstudiante: estudianteUser.id },
      });
      await prisma.estudianteCurso.create({
        data: { refCurso: curso.id, refEstudiante: estudianteUser2.id },
      });

      const response = await request(app)
        .get(`/api/estudiante-curso/curso/${curso.id}`)
        .set('Authorization', `Bearer ${profesorToken}`);

      expect(response.status).toBe(200);
      expect(response.body.mensaje).toBe(
        'Estudiantes del curso obtenidos con éxito'
      );
  ...
  describe('DELETE /api/estudiante-curso/eliminar/:refCurso/:refEstudiante', () => {
    it('Si el usuario no tiene permisos debe retornar 401', async () => {
      const response = await request(app)
        .delete(
          `/api/estudiante-curso/eliminar/${curso.id}/${estudianteUser.id}`
        )
        .set('Authorization', `Bearer ${estudianteToken}`);
